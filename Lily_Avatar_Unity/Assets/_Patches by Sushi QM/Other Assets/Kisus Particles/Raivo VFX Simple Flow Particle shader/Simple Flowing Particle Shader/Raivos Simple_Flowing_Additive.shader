...
////// Emissive:
                ...
                float2 node_6934 = ((_Distortion_var.r*_DIstortionPower)+(i.uv0+(node_8225.g*float2(_MainTextureScroll.r,_MainTextureScroll.g))));
                float4 _MainTexture_var = tex2D(_MainTexture,TRANSFORM_TEX(node_6934, _MainTexture));
                float3 emissive = (_Mask_var.rgb*(_MainTexture_var.rgb*i.vertexColor.rgb)*(lerp(float3(1,1,1),saturate(3.0*abs(1.0-2.0*frac(_Hue+float3(0.0,-1.0/3.0,1.0/3.0)))-1),_ColorSaturation)*_Power));
